...
const generateTicketPDF = async (ticket) => {
  ...
  const filePath = path.join(__dirname, `../tickets/${ticket.ticketNumber}.pdf`);
  const writeStream = fs.createWriteStream(filePath);
  doc.pipe(writeStream);

  doc.fontSize(25).text('Heritage Site Ticket', { align: 'center' });
  doc.moveDown();
  doc.fontSize(16).text(`Ticket Number: ${ticket.ticketNumber}`);
  ...
  const qrCodeData = `Ticket Number: ${ticket.ticketNumber}\nNumber of Visitors: ${ticket.numberOfVisitors}\nVisit Date: ${new Date(ticket.visitDate).toDateString()}`;
